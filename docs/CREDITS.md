# Crédits

Dark Dungeon Fantasy Boss Battle est un projet étudiant non commercial. Merci aux artistes qui partagent leur travail.

> **Règle** : chaque asset présent dans `public/assets/` a une ligne ici, ajoutée dans la même PR. Recopier la licence **exacte** indiquée sur la page de l'auteur. Voir [08-CONVENTIONS §4](08-CONVENTIONS.md#4-assets-et-licences).

## Graphismes

| Asset / pack | Auteur | Lien | Licence | Utilisé pour | Dans le repo ? |
|---|---|---|---|---|---|
| *ex. 16x16 DungeonTileset II* | 0x72 | https://0x72.itch.io/dungeontileset-ii | CC0 | Décors, héros | Oui |
| *ex. Monsters Creatures Fantasy* | LuizMelo | https://luizmelo.itch.io/monsters-creatures-fantasy | *à vérifier sur la page* | Gobelin, Squelette, Œil volant, Champignon | |
| *ex. Free Mythic Monsters* | Willibab | https://willibab.itch.io/free-mythic-monsters | *à vérifier sur la page* | | |
| | | | | | |

## Polices

| Police | Auteur | Lien | Licence |
|---|---|---|---|
| *ex. Press Start 2P* | CodeMan38 | https://fonts.google.com/specimen/Press+Start+2P | SIL Open Font License 1.1 |

## Sons et musiques

| Fichier | Auteur | Lien | Licence |
|---|---|---|---|
| | | | |

## Bibliothèques

| Bibliothèque | Licence |
|---|---|
| Phaser | MIT |
| React | MIT |
| @supabase/supabase-js | MIT |
| Vite | MIT |
| Vitest | MIT |

## Inspiration

- *PokeRogue* et la série *Pokémon* (Game Freak / Nintendo / The Pokémon Company) pour le principe de combat. Aucun asset de ces jeux n'est utilisé.
