...
- Taille de cadre fixée au sprint 1 : **64 × 64**, déclarée par `FRAME_SIZE` dans `src/game/scenes/PreloadScene.ts`.
- **Les assets sont générés par notre code** (`tools/art/`, `npm run assets`) : dessin en 32 × 32 puis agrandissement × 2,
  2 images par monstre (animation d'attente). Modifier un sprite = modifier `tools/art/monsters.mjs`, pas le PNG.
